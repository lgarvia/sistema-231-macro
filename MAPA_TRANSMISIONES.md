# MAPA DE TRANSMISIONES — SISTEMA 231

> **Actualizado:** 2026-09-12 — TASK_099, fase 5; topología y calibración de tesis conservadas

## Mapa activo

| Evento | Vector primario | Transmisión principal | Tesis |
|:---|:---:|:---|:---|
| [[Evento_E0_2026_08_16_Japon_Carry_Trade_y_Liquidez_Septiembre]] | V01 | ahorro japonés → duración soberana → repo/TGA | [[TESIS_01_Dominancia_Fiscal]]; [[TESIS_05_Tokenizacion_del_Colateral]] |
| [[Evento_E1_2026_06_15_Lloyds_War_Risk_Ormuz_BabelMandeb]] | V02 | seguro/fletes → energía y comercio → inflación | [[TESIS_04_Multipolaridad_Logistica]] |
| [[Evento_E0_2026_07_08_Grid_Stress_IA]] | V02 | generación/interconexión → centros de datos → CAPEX | [[TESIS_06_IA_como_silicio_y_energia]] |
| [[Evento_E0_2026_CoWoS_Capacity]] | V03 | packaging/HBM → aceleradores → inversión y red | [[TESIS_06_IA_como_silicio_y_energia]] |
| [[Evento_E1_2026_07_24_US_Tariff_Stack]] | V04 | aranceles → costes/volúmenes → industria e inflación | [[TESIS_02_Frictionless_Stabilization]] |

## Observatorios sin ficha activa

| Observatorio | Vive en | Trigger de reapertura |
|:---|:---|:---|
| Interferencia GNSS marítima | V02 | accidente atribuible, orden mandatoria o desvío físico |
| Pensiones y saldo contributivo | V05 / TESIS_03 | liquidación comparable que cruce un umbral fiscal |
| Zona gris en Taiwán | V06 | NOTAM, zona de exclusión o bloqueo material |

## Reglas

1. Un evento tiene un solo vector primario.
2. Los secundarios describen transmisión y no duplican la carga.
3. Una tesis no es fuente factual.
4. Un observatorio de fondo no cuenta como evento.
5. Las fichas cerradas preservan la memoria: [[EVENTOS_CERRADOS_LOG]].

## Próximos cruces

- Recompras de liquidez del Tesoro, Japón y ventana TGA/repo → V01/TESIS_01; no se imputan a TESIS_05 sin evidencia tokenizada.
- DOE y respuesta material a FERC tras la suspensión → V02/V03/TESIS_06; Eddystone queda cubierta por 202-26-40 hasta el 20-nov.
- Contraranceles canadienses efectivos y respuesta estadounidense Section 338; siguientes hitos 15 y 29-sep → V04/V05/TESIS_02.
- Ingresos TSMC de agosto consumidos; seguimiento de AP7, Rubin/CoWoS/HBM y CAPEX → V03/V02/TESIS_06.
- OPEP+ y EIA → V02/V04/TESIS_04.
- Subastas JGB y flujos MOF → V01/TESIS_01.
- Pensiones, vivienda, productividad y CAPEX europeo → V05/TESIS_03.
- Asamblea General de la ONU y agenda europea de sanciones → V06/V04/TESIS_04.
- FOMC/BoJ/BCE → V01/TESIS_01; la mera fecha no modifica estados ni carga.
- Unitree/Alibaba y métricas comparables de CAPEX China-Europa → V03/V05/TESIS_03/TESIS_06.

## Estado y horizonte

Carga **86,4** y tendencias según [[VECTOR_00_Indice]]. Ventanas solo en [[Radar_Eventos_2026_09]]; la lista de cruces es un mapa de seguimiento, no una afirmación de que todos tengan fila calendarizada ni un cambio de estado ex ante. La topología no cambia en W37: el aumento de 3,2 procede únicamente de la tendencia ↑ de Tariff Stack. La revisión de tesis tampoco altera enlaces causales: refuerza mecanismos parciales sin promover transmisiones secundarias a eventos o carga.
